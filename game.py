...
def show_question_art(kind: str) -> None:
    ...
    selected = arts.get(kind, arts["signal"])
    draw_frame([f"{BOLD}8-BIT КАРТИНКА{RESET}", *selected])


def play_challenges(level_name: str, challenges: list[Callable[[], int]]) -> int:
    points = 0
    total = len(challenges)
    for index, challenge in enumerate(challenges, start=1):
        draw_frame([f"{BOLD}{level_name}{RESET}", f"Задание {index}/{total}"])
        points += challenge()
    return points
...
def level1_garden(name: str) -> int:
    ...
    return play_challenges("Уровень 1 — Сад Светлячков", challenges)


def challenge_reeds() -> int:
    show_question_art("pond")
...
def level2_pond(name: str) -> int:
    ...
    return play_challenges("Уровень 2 — Пруд стрекоз", challenges)
...
def main() -> None:
    ...
    level = ask_choice(
        f"Выбери уровень (в каждом уровне по {TOTAL_CHALLENGES_PER_LEVEL} заданий):",
        {
